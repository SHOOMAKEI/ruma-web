...
function Edit() {
    // @ts-ignore
    const { regions, district } = usePage().props;
    const { data, setData, errors, put, processing } = useForm({
        name: (district as geopoliticalZone).name  || '',
        code_name: (district as geopoliticalZone).code_name ||'',
        region_id: (district as geopoliticalZone).region_id || '',
        longitude: (district as geopoliticalZone).longitude || '',
        latitude: (district as geopoliticalZone).latitude || ''
    });

    const defaultPosition= {
        lat: (district as geopoliticalZone).latitude ==0?9.081999:(district as geopoliticalZone).latitude,
        lng: (district as geopoliticalZone).longitude==0?8.675277:(district as geopoliticalZone).longitude
    }
    ...
    function handleSubmit(e: { preventDefault: () => void; }) {
        e.preventDefault();
        put(route('districts.update', (district as geopoliticalZone).id));
    }
