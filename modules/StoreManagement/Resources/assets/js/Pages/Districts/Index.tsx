...
function Index()  {
    const { districts, auth }:props = usePage().props
...
<CardWrapper>
        <table id="contract-data-table" className="table table-row-bordered gy-5 gs-7 border rounded">
            ...
            <tbody>
            {districts as props && (districts as props).map((group: geopoliticalZone)=>(
                <tr key={group.id}>
                    <td>{group.name}</td>
                        ...
                        <Dropdown>
                            ...
                            <Dropdown.Menu className="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold py-4 fs-6 w-275px">
                                <Dropdown.Item as={CustomDropdownMenuItem}>
                                    <InertiaLink href={route('districts.edit', group.id)} className="menu-link px-3" >Edit</InertiaLink>
                                </Dropdown.Item>
                                <Dropdown.Item as={CustomDropdownMenuItem}>
                                    <InertiaLink href={route('districts.destroy', group.id)} method="delete" className="menu-link px-3" >Delete</InertiaLink>
                                </Dropdown.Item>
