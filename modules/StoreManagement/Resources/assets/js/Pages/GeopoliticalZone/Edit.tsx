...
function Edit() {
    // @ts-ignore
    const { countries, geopolitical_zone } = usePage().props;
    const { data, setData, errors, put, processing } = useForm({
        name: (geopolitical_zone as geopoliticalZone).name  || '',
        code_name: (geopolitical_zone as geopoliticalZone).code_name ||'',
        country_id: (geopolitical_zone as geopoliticalZone).country_id || '',
        longitude: (geopolitical_zone as geopoliticalZone).longitude || '',
        latitude: (geopolitical_zone as geopoliticalZone).latitude || ''
    });

    const defaultPosition= {
        lat: (geopolitical_zone as geopoliticalZone).latitude ==0?9.081999:(geopolitical_zone as geopoliticalZone).latitude,
        lng: (geopolitical_zone as geopoliticalZone).longitude==0?8.675277:(geopolitical_zone as geopoliticalZone).longitude
    }
    ...
    function handleSubmit(e: { preventDefault: () => void; }) {
        e.preventDefault();
        put(route('geopolitical-zones.update', (geopolitical_zone as geopoliticalZone).id));
    }
...
Edit.layout = (page: boolean | React.ReactChild | React.ReactFragment | React.ReactPortal | null | undefined) => <Layout
    children={page}  title="Edit Geopolitical Zone"
 />;
